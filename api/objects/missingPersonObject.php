...
class MissingPersons{
  ...
  function calIDF(){
    ...
    function calIR(){
      ...
      // name add first array
      foreach ($this->name_array as $key => $value) {
        $query = "SELECT * FROM $this->table_name WHERE plost_id = '$key'";
        $stmt = $this->connection->prepare($query);
        $stmt-> execute();
      ...
      // city add second array
      foreach ($sim as $key => $value) {
        $key_plus = $key;
        if (!empty($this->city_array[$key])) {
          if ($this->city_array[$key] = $key && $value >= 0) {
